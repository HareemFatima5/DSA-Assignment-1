...
do {
    if (current->remaining_time <= 0) {
        ...
        current = current->next;
            ...
            int new_process_id;
            cout << "Enter new process ID: ";
            cin >> new_process_id;

            // if process id is -1 then continue
            if (new_process_id != -1) {
                insertNode(new_process_id);

                float new_remaining_time;
                cout << "Enter remaining time for process P" << new_process_id << ": ";
                cin >> new_remaining_time;
                
                remainingTime(new_process_id, new_remaining_time);
            }
